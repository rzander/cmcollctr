...
namespace RuckZuck_WCF
{
    class RZRestAPI
    {
        ...
        public static async void Feedback(string productName, string productVersion, string working, string userKey, string feedback)
        {
            ...
            await Task.Run(() =>
            {
                try
                {
                    ...
                    oClient.Headers.Add("AuthenticatedToken:" + Token);
                    oClient.Headers.Add("Content-Type:application/xml");
                    string sResult = oClient.DownloadString(sURL + "/rest/Feedback?name=" + WebUtility.UrlEncode(productName) + "&ver=" + WebUtility.UrlEncode(productVersion) + "&ok=" + working + "&user=" + WebUtility.UrlEncode(userKey) + "&text=" + WebUtility.UrlEncode(feedback));
                }
                catch { }
            });
        }
        ...
        public static List<string> GetCategories(List<GetSoftware> oSWList)
        {
            ...
            {
                lResult.AddRange(oSW.Categories.ToArray());
            }
